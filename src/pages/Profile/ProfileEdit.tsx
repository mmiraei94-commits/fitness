...
const ProfileEdit = ({ onClose }: IProfileEdit) => {
  ...
  const handleSave = () => {
    userData.setAge(formData.age);
    userData.setWeight(formData.weight);
    userData.setHeight(formData.height);
    userData.setGoal(
      formData.goal as "lose weight" | "maintain weight" | "gain muscle",
    );
    onClose();
  };
